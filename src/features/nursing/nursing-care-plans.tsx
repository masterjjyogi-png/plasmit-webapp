...
function SheetInput({
  ...
  return (
    <input
      className={cn("h-full w-full bg-transparent text-sm leading-snug outline-none placeholder:text-muted-foreground", className)}
      value={value}
...
function cloneTemplate(template: CarePlanTemplate): CarePlanTemplate {
  return {
    ...template,
    problems: template.problems.map((problem) => ({
      ...problem,
      goals: problem.goals.map((goal) => ({ ...goal, interventions: goal.interventions.map((intervention) => ({ ...intervention })) })),
    })),
  };
}

export function NursingCarePlanConfigurationPage() {
  const [templates, setTemplates] = React.useState<CarePlanTemplate[]>(initialTemplates.map(cloneTemplate));
  const [draft, setDraft] = React.useState<CarePlanTemplate>(cloneTemplate(initialTemplates[0]));

  function editTemplate(template: CarePlanTemplate) {
    setDraft(cloneTemplate(template));
  }

  function saveTemplate() {
    setTemplates((current) => current.map((template) => (template.id === draft.id ? cloneTemplate(draft) : template)));
  }

  function addNewCarePlan() {
    const next: CarePlanTemplate = { id: `tpl-${Date.now()}`, name: "New care plan", problems: [] };
    setTemplates((current) => [...current, next]);
    editTemplate(next);
  }

  function updateProblem(problemId: string, value: string) {
    setDraft((current) => ({ ...current, problems: current.problems.map((problem) => (problem.id === problemId ? { ...problem, name: value } : problem)) }));
  }
  ...
  function addProblem() {
    setDraft((current) => ({ ...current, problems: [...current.problems, { id: `problem-${Date.now()}`, name: "New problem", goals: [] }] }));
  }

  function addGoal(problemId: string) {
    setDraft((current) => ({
      ...current,
      problems: current.problems.map((problem) => problem.id === problemId ? { ...problem, goals: [...problem.goals, { id: `goal-${Date.now()}`, name: "New goal", interventions: [] }] } : problem),
    }));
  }
  ...
  function addIntervention(problemId: string, goalId: string) {
    setDraft((current) => ({
      ...
      problems: current.problems.map((problem) => problem.id === problemId ? {
        ...
        goals: problem.goals.map((goal) => goal.id === goalId ? {
          ...goal,
          interventions: [...goal.interventions, { id: `intervention-${Date.now()}`, name: "New intervention" }],
        } : goal),
      } : problem),
    }));
  }
  ...
  return (
    <NursingShell title="Care Plan Configurations" description="Configure care plans with problems, goals, and interventions." showSectionNav>
      <div className="overflow-auto bg-white p-2">
        <div className="min-w-[860px]">
          <table className="w-[780px] border-collapse bg-white">
            <tbody>
              <tr>
                <Cell colSpan={4}>Screen 1</Cell>
                <Cell />
              </tr>
              <tr>
                <Cell className="bg-[#4472c4] font-semibold text-black" colSpan={2}>Care plan configurations</Cell>
                <Cell />
                <Cell />
                <Cell />
              </tr>
              <tr><Cell colSpan={5} /></tr>
              <tr>
                <Cell className="bg-[#fce4d6] font-semibold" colSpan={2}>List of care plans</Cell>
                <Cell />
                <Cell />
                <Cell />
              </tr>
              {templates.map((template) => (
                <tr key={template.id}>
                  <Cell className="bg-[#fce4d6]">
                    <SheetButton onClick={() => editTemplate(template)}>{template.name}</SheetButton>
                  </Cell>
                  <Cell className="bg-[#fce4d6]">
                    <SheetButton onClick={() => editTemplate(template)}>Edit</SheetButton>
                  </Cell>
                  <Cell />
                  <Cell />
                  <Cell />
              ...
              <tr>
                <Cell className="bg-[#fce4d6]" colSpan={2}>
                  <SheetButton onClick={addNewCarePlan}>New row for adding new care plan</SheetButton>
                </Cell>
                <Cell />
                <Cell />
                <Cell />
              ...
              <tr><Cell colSpan={5} /></tr>
              <tr>
                <Cell className="bg-[#fce4d6] font-semibold">Screen 2</Cell>
                <Cell>When clicked on edit</Cell>
                <Cell />
                <Cell />
                <Cell />
              </tr>
              ...
              <tr>
                <Cell className="bg-[#4472c4] font-semibold text-black">Name of care plan</Cell>
                <Cell className="bg-[#8ea9db]">
                  <SheetButton onClick={addProblem}>Add problem</SheetButton>
                </Cell>
                <Cell className="bg-[#fce4d6]">
                  <SheetButton onClick={saveTemplate}>Save</SheetButton>
                </Cell>
                <Cell />
                <Cell />
              </tr>
              <tr>
                <Cell>
                  <SheetInput value={draft.name} onChange={(value) => setDraft((current) => ({ ...current, name: value }))} />
                </Cell>
                <Cell />
                <Cell />
                <Cell />
              ...
              {draft.problems.map((problem, problemIndex) => (
                <React.Fragment key={problem.id}>
                  <tr>
                    <Cell>
                      <SheetInput value={problem.name || `Problem ${problemIndex + 1}`} onChange={(value) => updateProblem(problem.id, value)} />
                    </Cell>
                    <Cell className="bg-[#a9d18e]">
                      <SheetButton onClick={() => addGoal(problem.id)}>Add goal</SheetButton>
                    </Cell>
                    <Cell />
                    <Cell />
                    <Cell />
                  ...
                  {problem.goals.map((goal, goalIndex) => (
                    <React.Fragment key={goal.id}>
                      <tr>
                        <Cell />
                        <Cell>
                          <SheetInput value={goal.name || `Goal ${goalIndex + 1}`} onChange={(value) => updateGoal(problem.id, goal.id, value)} />
                        </Cell>
                        <Cell className="bg-[#a9d18e]">
                          <SheetButton onClick={() => addIntervention(problem.id, goal.id)}>Add intervention</SheetButton>
                        </Cell>
                        <Cell />
                        <Cell />
                      </tr>
                      {goal.interventions.map((intervention, interventionIndex) => (
                        <tr key={intervention.id}>
                          ...
                          <Cell />
                          <Cell>
                            <SheetInput value={intervention.name || `Intervention ${interventionIndex + 1}`} onChange={(value) => updateIntervention(problem.id, goal.id, intervention.id, value)} />
                          </Cell>
                          <Cell />
                          <Cell />
                        </tr>
                      ...
                      <tr>
                        <Cell />
                        <Cell />
                        <Cell className="bg-[#ffd966]">
                          <SheetButton onClick={() => addIntervention(problem.id, goal.id)}>New row for intervention</SheetButton>
                        </Cell>
                        <Cell />
                        <Cell />
                      </tr>
                  ...
                  <tr>
                    <Cell />
                    <Cell className="bg-[#ffd966]">
                      <SheetButton onClick={() => addGoal(problem.id)}>New row for goal</SheetButton>
                    </Cell>
                    <Cell />
                    <Cell />
                    <Cell />
              ...
              <tr>
                <Cell className="bg-[#fce4d6]">
                  <SheetButton onClick={addProblem}>Problem {draft.problems.length + 1}</SheetButton>
                </Cell>
                <Cell className="bg-[#a9d18e]">
                  <SheetButton onClick={addProblem}>Add goal</SheetButton>
                </Cell>
                <Cell />
                <Cell />
                <Cell />
